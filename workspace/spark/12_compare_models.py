...
# ==========================================
print("\n" + "=" * 100)
print("                       FINAL MODEL COMPARISON REPORT (Hourly Data)")
print("=" * 100)
...
print(f"{'MAPE (lower is better)':<30} {lr_mape:>17.2f}% {lstm_mape:>9.2f}% {sarima_mape:>11.2f}% {hybrid_mape:>11.2f}%")
print("=" * 100)

print("\nNote: All models use the SAME hourly dataset and chronological train/test split.")
print("      RMSE values are directly comparable across all models.")
...
models = {
    ...
    "LSTM":              {"rmse": lstm_rmse, "mae": lstm_mae, "r2": lstm_r2, "mape": lstm_mape},
    "SARIMA":            {"rmse": sarima_rmse, "mae": sarima_mae, "r2": sarima_r2, "mape": sarima_mape},
    "Hybrid (SARIMA+LSTM)": {"rmse": hybrid_rmse, "mae": hybrid_mae, "r2": hybrid_r2, "mape": hybrid_mape},
}
...
mape_winner = min(models, key=lambda m: models[m]["mape"])
...
print(f"  Best MAPE : {mape_winner}  ({models[mape_winner]['mape']:.2f}%)")

scores = {name: 0 for name in models}
scores[rmse_winner] += 1
scores[mae_winner]  += 1
scores[r2_winner]   += 1
scores[mape_winner] += 1

overall = max(scores, key=scores.get)
print(f"\n  Overall Winner: {overall} ({scores[overall]}/4 metrics)")
